...
const DrugShortagePredictionPharmacy = () => {
  ...
  useEffect(() => {
    if (user && user._id) {
      setUserType(user.role === 'retailer' ? 'retailer' : 'pharmacy');
  ...
  const fetchShortagePredictions = async () => {
    try {
      ...
      const endpoint = userType === 'retailer' 
        ? `http://localhost:5000/api/predictions/retailer/${user._id}`
        : `http://localhost:5000/api/predictions/pharmacy/${user._id}`;
      ...
      console.log("response of prediction:", response.data);
    ...
    <div className="pharma-predictions-tab">
      <div className="pharma-card">
        <div className="pharma-card-header">
          <h5>{userType === 'retailer' ? 'Retailer' : 'Pharmacy'} Drug Shortage Predictions</h5>
